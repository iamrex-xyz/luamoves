...
import { Check, Sparkles, CheckCircle2, Home, Calendar, Building2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { MovingInfo } from "@/pages/Index";
import { useProfileSync } from "@/hooks/useProfileSync";

type BouwkundigeKeuringQuestionsDialogProps = {
  open: boolean;
...
type Step = "address" | "woningType" | "bouwjaar" | "voorkeursdatum" | "confirmation";

export const BouwkundigeKeuringQuestionsDialog = ({
  open,
  onOpenChange,
  movingInfo,
  onComplete,
  onCompleteTask,
}: BouwkundigeKeuringQuestionsDialogProps) => {
  ...
  const handleNext = async () => {
    if (currentStep === "address") {
      ...
      const data: Record<string, any> = {
        ...
        bouwkundigeKeuringVoorkeursdatum: voorkeursdatum || null,
      };
        ...
        {currentStep === "confirmation" ? (
          <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 text-center">
            ...
            <p className="text-muted-foreground mb-8 max-w-[280px]">
              Lua gaat aan de slag en komt snel terug met de beste bouwkundige keurders in jouw regio.
            </p>
          ...
          <>
            <div className="flex-1 overflow-y-auto px-6 py-5">
              <div className="flex gap-1 mb-6">
                {["address", "woningType", "bouwjaar", "voorkeursdatum"].map((step, idx) => (
                  <div 
                    key={step}
                    className={cn(
                      "h-1 flex-1 rounded-full transition-colors",
                      idx <= ["address", "woningType", "bouwjaar", "voorkeursdatum"].indexOf(currentStep)
                        ? "bg-primary"
                        : "bg-muted"
                    )}
                  />
                  ...
                  <div className="bg-muted/50 rounded-2xl rounded-tl-md px-4 py-3">
                    {currentStep === "address" && (
                      <p className="text-foreground">
                        Welke woning wil je laten keuren? 🏠
                      </p>
                    )}
                    ...
                    {currentStep === "bouwjaar" && (
                      <p className="text-foreground">
                        Wanneer is de woning gebouwd? Dit is belangrijk voor de inspectie. 📅
                      </p>
                    )}
                    {currentStep === "voorkeursdatum" && (
                      <p className="text-foreground">
                        Heb je een voorkeursdatum voor de keuring? (optioneel) 🗓️
                      </p>
                    )}
                  </div>
                </div>
